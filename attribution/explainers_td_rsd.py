...
class OUR_TD:
    ...
    def attribute_trend_residual_segments(
        ...
        resid_s     = (inputs_s - trend_s).contiguous()   # residual waypoint R = x - T
        fixed_inputs = inputs_s.detach()
 
        # === Residual-first path:  c -> R -> x ===
        # Phase 1: c -> R  (mask==1 위치에서 baseline → residual, 방향 R - c)
        resid_attr = self._ig_phase(
            baselines_s, resid_s, time_mask, fixed_inputs,
            alphas, targets, return_all,
            n_samples, n_alphas, B, T, D, alpha_chunk)
 
        # Phase 2: R -> R + T = x  (mask==1 위치에서 residual → input, 즉 + trend, 방향 x - R = T)
        trend_attr = self._ig_phase(
            resid_s, inputs_s, time_mask, fixed_inputs,
            alphas, targets, return_all,
            n_samples, n_alphas, B, T, D, alpha_chunk)
